/* Interior registry source: floating-label · https://www.interior.dev/docs/floating-label */
/* eslint-disable react-hooks/immutability, react-hooks/set-state-in-effect -- registry source synchronizes DOM input state */
"use client";
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="relative pt-[20px]">
        <div
          className={`relative h-11 rounded-[10px] transition-colors duration-150 ${
            invalid
              ? "bg-red-50 dark:bg-red-950/30"
              : focused
                ? "bg-stone-100 dark:bg-[#252522]"
                : "bg-stone-100/70 dark:bg-[#1D1D1A]"
          } ${disabled ? "opacity-55" : ""}`}
        >
          <input
          ref={attach}
          ...
          onChange={(event) => {
            ...
            className="absolute inset-0 h-full w-full rounded-[9px] bg-transparent px-3 py-0 text-[13px] leading-[20px] text-stone-700 outline-none focus-visible:bg-stone-200/50 focus-visible:outline-none disabled:cursor-not-allowed dark:text-stone-200 dark:focus-visible:bg-white/[0.04]"
          />
        </div>
